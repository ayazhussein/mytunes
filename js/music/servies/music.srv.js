        ...
        return new Observable_1.Observable(function (observable) {
            ...
            _this.http.get(url)
                ...
                results.albummatches.album.forEach(function (data) {
                    albums.unshift(new album_1.Album(data["mbid"], data["name"], data["artist"], data["url"], data["image"]));
                });
                var pager = new pager_1.Pager(results["opensearch:itemsPerPage"], results["opnesearch:startIndex"], results["opensearch.totalResults"]);
                return {
                    albums: albums,
                    pager: pager
                };
